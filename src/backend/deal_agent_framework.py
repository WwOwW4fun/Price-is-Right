...
class DealAgentFramework:
    DB = ROOT_DIR / "products_vectorstore"
    MEMORY_FILENAME = SRC_DIR / "memory.json"
    ...
    def read_memory(self) -> List[Opportunity]:
        if self.MEMORY_FILENAME.exists():
            with self.MEMORY_FILENAME.open("r") as file:
                data = json.load(file)
    ...
    def write_memory(self) -> None:
        ...
        with self.MEMORY_FILENAME.open("w") as file:
    ...
    def reset_memory(cls) -> None:
        ...
        if cls.MEMORY_FILENAME.exists():
            with cls.MEMORY_FILENAME.open("r") as file:
                data = json.load(file)
        truncated = data[:2]
        with cls.MEMORY_FILENAME.open("w") as file:
